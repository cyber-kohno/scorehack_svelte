...
const useBuilderModulate = (lastStore: StoreProps) => {
    ...
    const get = (): CommandRegistUtil.FuncProps[] => {
        ...
        const getCurrDispValue = (data: StoreOutline.DataModulate) => `[${data.method}${data.val ? ' ' + data.val : ''}]`;
        ...
        return [
            {
                ...defaultProps,
                funcKey: 'domm',
                args: [{ name: 'value', getCandidate: () => StoreOutline.DommVals.map(v => v.toString()) }],
                callback: (args) => {
